...
template <typename T>
__global__ void layer_norm_kernel(const T* __restrict__ input, const T* __restrict__ gamma,
    ...
    if (threadIdx.x == 0)
        s_inv_std = rsqrtf(var_sum / hidden_size + eps);
    __syncthreads();
    ...
    for (int i = threadIdx.x; i < hidden_size; i += blockDim.x) {
        ...
        row_output[i] =
            static_cast<T>(normalized * static_cast<float>(gamma[i]) + static_cast<float>(beta[i]));
    }
}

template <>
void layer_norm<float>(const float* input, const float* gamma, const float* beta, float* output,
                       int batch, int hidden_size, float eps, cudaStream_t stream) {
    int block_size = 256;
    layer_norm_kernel<float>
        <<<batch, block_size, 0, stream>>>(input, gamma, beta, output, hidden_size, eps);
    CUDA_CHECK_LAST();
}

template <>
void layer_norm<__half>(const __half* input, const __half* gamma, const __half* beta,
                        __half* output, int batch, int hidden_size, float eps,
                        cudaStream_t stream) {
    int block_size = 256;
