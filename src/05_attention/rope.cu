...
template <typename T>
__global__ void apply_rope_kernel(T* __restrict__ query, T* __restrict__ key,
                                  const float* __restrict__ cos_cache,
                                  const float* __restrict__ sin_cache, int batch, int num_heads,
...
template <>
void apply_rope<float>(float* query, float* key, int batch, int num_heads, int seq_len,
                       int head_dim, const float* cos_cache, const float* sin_cache,
                       cudaStream_t stream) {
    int total = batch * num_heads * seq_len * (head_dim / 2);
    int block_size = 256;
    ...
    apply_rope_kernel<float><<<grid_size, block_size, 0, stream>>>(
        query, key, cos_cache, sin_cache, batch, num_heads, seq_len, head_dim);
    CUDA_CHECK_LAST();
}

template <>
void apply_rope<__half>(__half* query, __half* key, int batch, int num_heads, int seq_len,
                        int head_dim, const float* cos_cache, const float* sin_cache,
                        cudaStream_t stream) {
    int total = batch * num_heads * seq_len * (head_dim / 2);
    int block_size = 256;
