...
template <typename T>
__global__ void transpose_naive_kernel(const T* __restrict__ input, T* __restrict__ output,
                                       int rows, int cols) {
...
template <typename T>
__global__ void transpose_shared_kernel(const T* __restrict__ input, T* __restrict__ output,
                                        int rows, int cols) {
...
template <typename T>
__global__ void transpose_shared_padded_kernel(const T* __restrict__ input, T* __restrict__ output,
                                               int rows, int cols) {
...
template <>
void transpose<float, TransposeOpt::Naive>(const float* input, float* output, int rows, int cols,
                                           cudaStream_t stream) {
...
template <>
void transpose<float, TransposeOpt::SharedMemory>(const float* input, float* output, int rows,
...
template <>
void transpose<float, TransposeOpt::SharedMemPadded>(const float* input, float* output, int rows,
                                                     int cols, cudaStream_t stream) {
    dim3 block(TILE_DIM, BLOCK_ROWS);
    dim3 grid((cols + TILE_DIM - 1) / TILE_DIM, (rows + TILE_DIM - 1) / TILE_DIM);
...
template <>
void transpose<__half, TransposeOpt::SharedMemPadded>(const __half* input, __half* output, int rows,
                                                      int cols, cudaStream_t stream) {
    dim3 block(TILE_DIM, BLOCK_ROWS);
    dim3 grid((cols + TILE_DIM - 1) / TILE_DIM, (rows + TILE_DIM - 1) / TILE_DIM);
