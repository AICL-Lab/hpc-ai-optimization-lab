...
__global__ void fp8_scale_kernel(const float* __restrict__ input, float* __restrict__ output,
                                 float scale, int n) {
...
void fp8_scale(const float* input, float* output, float scale, int n, cudaStream_t stream) {
    int block_size = 256;
