...
using hpc::reference::cpu_softmax;
...
RC_GTEST_PROP(SoftmaxTest, OutputProperties, ()) {
    ...
    const auto expected = cpu_softmax(input, batch, seq_len);
    ...
    EXPECT_TRUE(hpc::test::vectors_almost_equal(result, expected, 1e-3f, 1e-4f));
}

TEST(SoftmaxTest, BasicTest) {
    std::vector<float> input = {1.0f, 2.0f, 3.0f, 4.0f};
    const auto expected = cpu_softmax(input, 1, 4);
